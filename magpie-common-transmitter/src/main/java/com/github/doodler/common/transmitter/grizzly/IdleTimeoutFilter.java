...
/**
 * The Filter is responsible for tracking {@link Connection} activity and closing {@link Connection}
 * once it becomes idle for certain amount of time. Unlike {@link ActivityCheckFilter}, this Filter
 * assumes {@link Connection} is idle, when no event is being executed on it. But if some event
 * processing was suspended - this Filter still assumes {@link Connection} is active.
 * 
...
public class IdleTimeoutFilter extends BaseFilter {
    ...
    private static final Attribute<IdleRecord> IDLE_ATTR = Grizzly.DEFAULT_ATTRIBUTE_BUILDER
            .createAttribute(IDLE_ATTRIBUTE_NAME, new NullaryFunction<IdleRecord>() {
    ...
    public IdleTimeoutFilter(final DelayedExecutor executor, final long timeout,
    ...
    public IdleTimeoutFilter(final DelayedExecutor executor, final long timeout,
            final TimeUnit timeUnit, final TimeoutHandler handler) {

        this(executor, new DefaultWorker(handler),
     ...
     */
    public static void setCustomTimeout(final Connection connection, final long timeout,
        ...
        public void onComplete(final FilterChainContext ctx) {
            ...
            if (idleRecord.isClosed || IdleRecord.counterUpdater.decrementAndGet(idleRecord) == 0) {
                ...
                if (idleRecord.isClosed) {
                    ...
                    timeoutToSet =
                            timeout == FOREVER ? FOREVER : System.currentTimeMillis() + timeout;
                }

                IdleRecord.timeoutMillisUpdater.compareAndSet(idleRecord, FOREVER_SPECIAL,
                        timeoutToSet);
            }
        }
